...
import { SELECTABLE_PLANE_KEYS, VEHICLE_KEYS, VEHICLE_META } from '../../src/world/identity.js';
...
test('세 그룹이 등록부와 같은 수를 담는다', () => {
  ...
  assert.deepEqual(counts, { flight: SELECTABLE_PLANE_KEYS.length, car: VEHICLE_KEYS.length, walk: 1 });
  assert.ok(counts.flight >= 6 && counts.car >= 6, `항공기 ${counts.flight} 차량 ${counts.car}`);
});

test('키 목록이 identity 와 어긋나지 않는다', () => {
  assert.deepEqual(RIDE_GROUPS[0].rides.map((ride) => ride.key), SELECTABLE_PLANE_KEYS);
  assert.equal(rideOf('flight', 'jet'), null, '라이트 제트는 선택 카드가 없다');
  assert.match(rideOf('flight', 'helicopter').note, /기관총 2정/);
...
test('항공기 막대는 기종별 성능을 따라간다', () => {
  const fighter = rideOf('flight', 'fighter');
  const bomber = rideOf('flight', 'bomber'), prop = rideOf('flight', 'prop');
  assert.ok(fighter.bars.speed > bomber.bars.speed);
  ...
  assert.ok(prop.bars.stability > fighter.bars.stability, '실속 속도가 낮은 프로펠러기가 더 안정적이다');
});
