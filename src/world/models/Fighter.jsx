...
const FIN_CANT = 0.28;
const PAINT = '#8e979b', PAINT_DARK = '#6f797e', PAINT_DEEP = '#5a646a', METAL = '#394649', GLASS = '#385c6d';
...
export default function Fighter({ firstPerson = false }) {
  const geometries = useMemo(() => {
    ...
    wingShape.moveTo(-1.1, -0.8); wingShape.lineTo(-5.3, 3.4); wingShape.lineTo(-5.3, 4.1); wingShape.lineTo(-1.1, 4.6);
    wingShape.lineTo(1.1, 4.6); wingShape.lineTo(5.3, 4.1); wingShape.lineTo(5.3, 3.4); wingShape.lineTo(1.1, -0.8); wingShape.closePath();
    const wing = new THREE.ExtrudeGeometry(wingShape, { depth: 0.2, bevelEnabled: false });
    wing.rotateX(HALF_PI);

    const lerxShape = new THREE.Shape();
    lerxShape.moveTo(0.65, -4.6); lerxShape.lineTo(2.0, -0.8); lerxShape.lineTo(0.65, -0.8); lerxShape.closePath();
    const lerx = new THREE.ExtrudeGeometry(lerxShape, { depth: 0.12, bevelEnabled: false });
    lerx.rotateX(HALF_PI);

    const stabShape = new THREE.Shape();
    stabShape.moveTo(-1.0, 4.4); stabShape.lineTo(-3.1, 6.1); stabShape.lineTo(-3.1, 6.7); stabShape.lineTo(-1.0, 7.0);
    stabShape.lineTo(1.0, 7.0); stabShape.lineTo(3.1, 6.7); stabShape.lineTo(3.1, 6.1); stabShape.lineTo(1.0, 4.4); stabShape.closePath();
    const stabilizer = new THREE.ExtrudeGeometry(stabShape, { depth: 0.15, bevelEnabled: false });
    stabilizer.rotateX(HALF_PI);

    const finShape = new THREE.Shape();
    finShape.moveTo(3.2, 0); finShape.lineTo(5.4, 3.0); finShape.lineTo(6.3, 3.0); finShape.lineTo(6.8, 0); finShape.closePath();
  ...
  <StaticBatch>
    <mesh position={[0, 0, -6.7]} rotation={[-HALF_PI, 0, 0]} castShadow><coneGeometry args={[0.6, 2.6, 10]} /><meshStandardMaterial color={PAINT_DARK} roughness={0.6} /></mesh>
    ...
    <Block position={[0, 0, -3.6]} scale={[1.3, 1.2, 3.8]} color={PAINT} />
    <Block position={[0, -0.05, 1]} scale={[2.2, 1.4, 5.6]} color={PAINT} />
    <Block position={[0, 0, 5]} scale={[2.4, 1.2, 2.6]} color={PAINT_DARK} />
    <Block position={[0, 0.7, 1.2]} scale={[0.9, 0.45, 5.4]} color={PAINT} />
    ...
    <mesh position={[0, -1.6, -4.6]} rotation={[0, 0, HALF_PI]} castShadow><cylinderGeometry args={[0.3, 0.3, 0.24, 12]} /><meshStandardMaterial color="#2a3134" roughness={0.9} /></mesh>
    <Block position={[0.32, -1.0, -4.6]} scale={[0.06, 0.9, 1.2]} color={PAINT} />

    {[-1, 1].map((side) => <group key={side}>
      <Block position={[side * 0.75, -0.05, -3.6]} scale={[0.35, 0.5, 3.8]} color={PAINT_DARK} />
      ...
      <Block position={[side * 1.55, -0.35, 0.2]} scale={[0.9, 1.1, 3.6]} color={PAINT_DARK} />
      <Block position={[side * 1.55, -0.35, -1.62]} scale={[0.78, 0.96, 0.1]} color="#1d2427" />
      ...
      <mesh position={[side * 0.7, -0.05, 6.4]} rotation={[HALF_PI, 0, 0]} castShadow><cylinderGeometry args={[0.55, 0.5, 1.2, 10]} /><meshStandardMaterial color={METAL} metalness={0.7} roughness={0.4} flatShading /></mesh>
      <mesh position={[side * 0.7, -0.05, 6.92]}><torusGeometry args={[0.4, 0.07, 6, 10]} /><meshStandardMaterial color="#1a1f22" metalness={0.5} roughness={0.6} /></mesh>
      <mesh position={[side * 0.7, -0.05, 6.9]}><circleGeometry args={[0.42, 10]} /><meshStandardMaterial color="#0f1214" /></mesh>

      <Block position={[side * 1.5, -1.2, 1.8]} scale={[0.16, 1.1, 0.16]} color="#78898c" />
      <mesh position={[side * 1.5, -1.54, 1.8]} rotation={[0, 0, HALF_PI]} castShadow><cylinderGeometry args={[0.36, 0.36, 0.3, 12]} /><meshStandardMaterial color="#2a3134" roughness={0.9} /></mesh>
  ...
  <group visible={!firstPerson}>
    ...
    <Block position={[0, 0.95, -3.3]} scale={[1.5, 0.06, 0.1]} color={PAINT_DEEP} />
    <mesh position={[0, 1.02, -3.1]}><sphereGeometry args={[0.2, 8, 6]} /><meshStandardMaterial color={PAINT_DEEP} roughness={0.5} /></mesh>
      <mesh position={[0, 0.75, -3.3]} scale={[0.7, 0.7, 1.9]} castShadow><sphereGeometry args={[1, 12, 8]} /><meshStandardMaterial color={GLASS} metalness={0.3} roughness={0.22} /></mesh>
