  ...
  fragmentShader: `
    ...
    void main() {
      ...
      if (isRoadX || isRoadZ) {
        ...
        if (isRoadZ&&abs(dz+1.5)<0.05&&fz>0.5) color=laneColor;
        float modGx = mod(round(gx), uRoadInterval);
        float dToNSRoad = min(modGx, uRoadInterval - modGx);
        float modGz = mod(round(gz), uRoadInterval);
        float dToEWRoad = min(modGz, uRoadInterval - modGz);
        if (isRoadX && !isRoadZ && dToEWRoad < 1.3) {
          float stripe = step(0.5, fract(ax * 0.22));
          if (stripe < 0.5) color = uIsNight ? vec3(0.36,0.36,0.36) : vec3(0.88,0.88,0.88);
        }
        if (isRoadZ && !isRoadX && dToNSRoad < 1.3) {
          float stripe = step(0.5, fract(az * 0.22));
          if (stripe < 0.5) color = uIsNight ? vec3(0.36,0.36,0.36) : vec3(0.88,0.88,0.88);
        }
        float sidewalkOuter = uGridSize * 0.47;
        float sidewalkInner = uGridSize * 0.37;
        vec3 sidewalkColor  = uIsNight ? vec3(0.065,0.065,0.085) : vec3(0.71,0.69,0.65);
        if (isRoadX && dx > sidewalkInner && dx < sidewalkOuter) color = sidewalkColor;
        if (isRoadZ && dz > sidewalkInner && dz < sidewalkOuter) color = sidewalkColor;
      }
      if (uIsNight) {
        for(int i=0; i<30; i++) {
...
function Cars({ timeOfDay }) {
  const isNight = timeOfDay === 'night';
  // Roads every 168 units (GRID_SIZE=56 * ROAD_INTERVAL=3)
  const ROAD_POS = [-336, -168, 0, 168, 336];
  const colors = ['#CC3333','#3366CC','#DDAA22','#EEEEEE','#44AA66','#AA44CC','#DD6622'];

  // E-W cars: move along X axis, fixed Z position
  const EW_CARS = useMemo(() => {
    const cars = [];
    ROAD_POS.forEach((rz, ri) => {
      for (let i = 0; i < 5; i++) {
        ...
        cars.push({
          ...
          color: colors[(ri * 5 + i) % colors.length],
        });
      }
    });
    return cars;
  ...
  // N-S cars: move along Z axis, fixed X position
  const NS_CARS = useMemo(() => {
    const cars = [];
    ROAD_POS.forEach((rx, ri) => {
      for (let i = 0; i < 5; i++) {
        const dir = seededRand(ri * 43 + i * 23 + 100) > 0.5 ? 1 : -1;
        cars.push({
          rx: rx + (dir > 0 ? 3.5 : -3.5),
          startZ: (seededRand(ri * 11 + i * 7 + 200) * 2 - 1) * 900,
          speed: dir * (55 + seededRand(ri * 17 + i + 300) * 55),
          color: colors[(ri * 5 + i + 10) % colors.length],
        });
      }
    });
    return cars;
  ...
  const ewPosRef = useRef(EW_CARS.map(c => c.startX));
  const nsPosRef = useRef(NS_CARS.map(c => c.startZ));
  const ewGroupRefs = useRef([]);
  const nsGroupRefs = useRef([]);

  useFrame((_, delta) => {
    EW_CARS.forEach((car, i) => {
      ewPosRef.current[i] += car.speed * delta;
      if (ewPosRef.current[i] > 1000) ewPosRef.current[i] = -1000;
      if (ewPosRef.current[i] < -1000) ewPosRef.current[i] = 1000;
      if (ewGroupRefs.current[i]) ewGroupRefs.current[i].position.x = ewPosRef.current[i];
    });
    NS_CARS.forEach((car, i) => {
      nsPosRef.current[i] += car.speed * delta;
      if (nsPosRef.current[i] > 1000) nsPosRef.current[i] = -1000;
      if (nsPosRef.current[i] < -1000) nsPosRef.current[i] = 1000;
      if (nsGroupRefs.current[i]) nsGroupRefs.current[i].position.z = nsPosRef.current[i];
    });
  });
  ...
  return (
    <>
      {EW_CARS.map((car, i) => (
        <group key={`ew-${i}`} ref={el => { ewGroupRefs.current[i] = el; }}
      ...
      {NS_CARS.map((car, i) => (
        <group key={`ns-${i}`} ref={el => { nsGroupRefs.current[i] = el; }}
          position={[car.rx, 1.2, car.startZ]}
          rotation={[0, car.speed > 0 ? Math.PI * 0.5 : -Math.PI * 0.5, 0]}>
          <mesh>
            <boxGeometry args={[5.5, 1.8, 2.6]} />
            <meshStandardMaterial color={car.color} roughness={0.4} metalness={0.25} />
          </mesh>
...
function getTierWidth(t) {
  if (t==='champion') return 17.0;
  if (t==='master')   return 15.5;
  if (t==='diamond')  return 13.5;
  if (t==='platinum') return 12.0;
  if (t==='gold')     return 11.0;
  if (t==='silver')   return 10.0;
  return 9.0;
}
...
function Buildings({ data, timeOfDay, cameraDistRef }) {
  ...
  const corniceRefs  = useRef([null, null, null, null]);
  const lobbyRefs    = useRef([null, null, null, null]);
  ...
  useEffect(() => {
    groups.forEach((grpData, gi) => {
      ...
      const cornice= corniceRefs.current[gi];
      const lobby  = lobbyRefs.current[gi];
      ...
      grpData.forEach((b, i) => {
        ...
        // 직사각형 평면 변형 — 건물마다 독립적인 X/Z 비율
        const wX = w * (0.75 + seededRand(i * 11.7 + gi * 3.14) * 0.65);
        const wZ = w * (0.75 + seededRand(i *  7.31 + gi * 5.17) * 0.65);
        // 메인 바디
        dummy.position.set(b.x, sh/2, b.z);
        dummy.scale.set(wX, sh, wZ);
        ...
        if (extra) {
          if (gi===1) {
            const ph=sh*0.08; dummy.position.set(b.x,sh+ph/2,b.z); dummy.scale.set(wX*0.55,ph,wZ*0.55);
          } else if (gi===2) {
            const ch=sh*0.08; dummy.position.set(b.x,sh+ch/2,b.z); dummy.scale.set(wX*0.45,ch,wZ*0.45);
          } else if (gi===3) {
        ...
        // 코니스 캡 — 건물 정상부 돌출 슬래브
        if (cornice) {
          dummy.position.set(b.x, sh + 0.55, b.z);
          dummy.scale.set(wX + 1.8, 1.1, wZ + 1.8);
          dummy.rotation.set(0,0,0); dummy.updateMatrix();
          cornice.setMatrixAt(i, dummy.matrix);
          col.set(getTierColor(t)); col.multiplyScalar(0.72);
          cornice.setColorAt(i, col);
          col.set(getTierColor(t));
        }
        // 로비 포디엄 — 건물 하부 넓은 기단
        if (lobby) {
          const lbH = Math.min(8, sh * 0.12);
          dummy.position.set(b.x, lbH / 2, b.z);
          dummy.scale.set(wX * 1.16, lbH, wZ * 1.16);
          dummy.rotation.set(0,0,0); dummy.updateMatrix();
          lobby.setMatrixAt(i, dummy.matrix);
        }
      });
      ...
      if (cornice) { cornice.instanceMatrix.needsUpdate = true; if (cornice.instanceColor) cornice.instanceColor.needsUpdate = true; }
      if (lobby) lobby.instanceMatrix.needsUpdate = true;
    });
  }, [groups, dummy, col]);
      ...
      {groups.map((grpData, gi) => {
        ...
        return (
          <React.Fragment key={gi}>
            ...
            {/* 코니스 캡 — 정상부 돌출 슬래브 */}
            <instancedMesh ref={mesh=>{ if(mesh&&!mesh.instanceColor) mesh.instanceColor=new THREE.InstancedBufferAttribute(new Float32Array(grpData.length*3).fill(0.5),3); corniceRefs.current[gi]=mesh; }} args={[null,null,grpData.length]} frustumCulled={false}>
              <boxGeometry />
              <meshStandardMaterial vertexColors roughness={0.60} metalness={0.12} />
            </instancedMesh>
            {/* 로비 포디엄 — 건물 하부 기단 */}
            <instancedMesh ref={mesh=>{ lobbyRefs.current[gi]=mesh; }} args={[null,null,grpData.length]} frustumCulled={false}>
              <boxGeometry />
              <meshStandardMaterial color={isNight?'#1c1c24':'#8e8e94'} roughness={0.82} metalness={0.05} />
            </instancedMesh>
          </React.Fragment>
        );
      })}
...
function TrafficLights({ timeOfDay }) {
  const isNight = timeOfDay === 'night';
  const ROAD_POS = [-336, -168, 0, 168, 336];

  const { allPos, greenPos, redPos } = useMemo(() => {
    const allPos = [], greenPos = [], redPos = [];
    ROAD_POS.forEach(rx => {
      ROAD_POS.forEach(rz => {
        if (rz > 90 && rz < 220) return; // 강 구간 제외
        [[-1,1],[1,1],[1,-1],[-1,-1]].forEach(([sx,sz], ci) => {
          const p = { x: rx + sx * 11, z: rz + sz * 11 };
          allPos.push(p);
          if (ci % 2 === 0) greenPos.push(p); else redPos.push(p);
        });
      });
    });
    return { allPos, greenPos, redPos };
  }, []);
  ...
  useEffect(() => {
    if (poleRef.current) {
      allPos.forEach((p, i) => {
        dummy.position.set(p.x, 5, p.z);
        dummy.scale.set(1, 1, 1); dummy.rotation.set(0,0,0); dummy.updateMatrix();
        poleRef.current.setMatrixAt(i, dummy.matrix);
      });
      poleRef.current.instanceMatrix.needsUpdate = true;
    }
    [{ ref: greenRef, positions: greenPos }, { ref: redRef, positions: redPos }].forEach(({ ref, positions }) => {
      if (!ref.current || positions.length === 0) return;
      positions.forEach((p, i) => {
        dummy.position.set(p.x, 10.2, p.z);
        dummy.scale.set(1, 1, 1); dummy.rotation.set(0,0,0); dummy.updateMatrix();
        ref.current.setMatrixAt(i, dummy.matrix);
      });
      ref.current.instanceMatrix.needsUpdate = true;
    });
  }, [allPos, greenPos, redPos, dummy]);

  if (allPos.length === 0) return null;
  return (
    <>
      <instancedMesh ref={poleRef} args={[null, null, allPos.length]} frustumCulled={false}>
        <cylinderGeometry args={[0.22, 0.30, 10, 6]} />
        <meshStandardMaterial color={isNight ? '#111' : '#555'} roughness={0.75} />
      </instancedMesh>
      {greenPos.length > 0 && (
        <instancedMesh ref={greenRef} args={[null, null, greenPos.length]} frustumCulled={false}>
          <boxGeometry args={[1.1, 2.6, 0.7]} />
          <meshStandardMaterial color="#00BB44" emissive={isNight ? '#00FF66' : '#000000'} emissiveIntensity={isNight ? 2.0 : 0} roughness={0.3} />
        </instancedMesh>
      )}
      {redPos.length > 0 && (
        <instancedMesh ref={redRef} args={[null, null, redPos.length]} frustumCulled={false}>
          <boxGeometry args={[1.1, 2.6, 0.7]} />
          <meshStandardMaterial color="#CC2200" emissive={isNight ? '#FF3300' : '#000000'} emissiveIntensity={isNight ? 2.0 : 0} roughness={0.3} />
        </instancedMesh>
      )}
    </>
  );
...
export default function WorldPage() {
  ...
  const gShader = useMemo(() => ({
    ...
    uniforms: { ...GroundShaderMaterial.uniforms, uIsNight:{ value:isNight }, uGridSize:{ value:56.0 }, uRoadInterval:{ value:3.0 }, uLamps: { value: LAMP_POSITIONS } }
  }), [LAMP_POSITIONS]);
      ...
      <div className="w-screen h-screen relative bg-[#05050a] overflow-hidden select-none touch-none">
        ...
        <Canvas shadows camera={{ fov: 45, near: 0.5, far: 30000 }} gl={{ antialias: true }} dpr={QUALITY_DPR[quality]}>
          <Suspense fallback={null}>
            ...
            <TrafficLights timeOfDay={timeOfDay} />
