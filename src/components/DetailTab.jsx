    ...
    <div className="stat-grid">
      {cards.map(({ label, value, icon }) => (
        <div key={label} className="stat-card p-4 sm:p-5">
          <div className="flex items-center gap-1.5 text-[var(--text-tertiary)] mb-2">{icon}<span className="text-[10px] uppercase tracking-wider font-semibold">{label}</span></div>
          <div className="text-xl sm:text-2xl font-black text-[var(--accent)]">{value}</div>
        </div>
      ))}
    </div>
  );
