...
function StrengthCard({ stats, characters }) {
  const items = useMemo(() => {
  ...
  return (
    <div className="glass-card-sm p-4 mb-4">
      <h3 className="text-[11px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider mb-3 flex items-center gap-1.5">
        <span className="text-[var(--accent)]">⚡</span> 주요 지표
      </h3>
      <div className="grid grid-cols-3 gap-3">
        {items.map(s => (
          <div key={s.key} className="flex flex-col gap-0.5">
            <span className="text-[10px] text-[var(--text-tertiary)]">{s.label}</span>
            <span className="text-[15px] font-black text-[var(--accent)] leading-tight">
              {s.value}
              <span className="text-[10px] font-medium text-[var(--text-tertiary)] ml-0.5">{s.unit}</span>
            </span>
            {s.sub && <span className="text-[10px] text-[var(--text-tertiary)] truncate">{s.sub}</span>}
          </div>
        ))}
      </div>
    </div>
...
export default function StatsTab({ stats, characters }) {
  ...
  return (
    <div className="space-y-4">
      <StrengthCard stats={stats} characters={characters} />
      {/* 심화 수치 3종 */}
      {advanced && (
