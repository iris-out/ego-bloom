...
function Rank1Icon({ p }) {
  return (
    <>
      <style>{`
        @keyframes ${p}breathe { 0%, 100% { opacity: 0.6; transform: scale(0.95); } 50% { opacity: 1; transform: scale(1.05); } }
        .${p}aura { mix-blend-mode: color-dodge; animation: ${p}breathe 2s ease infinite; transform-origin: center; }
      `}</style>
      <defs>
        <linearGradient id={`${p}gradFlare`} x1="50%" y1="0%" x2="50%" y2="100%">
          <stop offset="0%" stopColor="#FFF"/>
          <stop offset="30%" stopColor="#FFD700"/>
          <stop offset="100%" stopColor="#FF6600"/>
        </linearGradient>
        <radialGradient id={`${p}radialFlare`} cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#FF6600"/>
            <stop offset="100%" stopColor="transparent"/>
        </radialGradient>
      </defs>
      <circle cx="50" cy="50" r="60" fill={`url(#${p}radialFlare)`} className={`${p}aura`} />
      <path d="M 5 85 L 95 85 L 100 40 L 75 55 L 50 10 L 25 55 L 0 40 Z" fill={`url(#${p}gradFlare)`} filter="drop-shadow(0 0 15px #FFD700)" />
      <path d="M 18 78 L 82 78 L 86 50 L 70 60 L 50 25 L 30 60 L 14 50 Z" fill="#FFF" opacity="0.8" />
      <polygon points="50,12 55,20 50,28 45,20" fill="#FFF" />
      <rect x="20" y="72" width="60" height="4" fill="#FFF" opacity="0.3" />
    </>
  );
}

function RankTop10Icon({ p }) {
  return (
    <>
      <defs>
        <linearGradient id={`${p}gradHolo`} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#00F0FF"/>
          <stop offset="50%" stopColor="#FF00A0"/>
          <stop offset="100%" stopColor="#FFD700"/>
        </linearGradient>
      </defs>
      <path d="M 5 85 L 95 85 L 100 40 L 75 55 L 50 10 L 25 55 L 0 40 Z" fill="#F8FAFC" filter="drop-shadow(0 0 25px rgba(255,255,255,0.6))" />
      <path d="M 18 78 L 82 78 L 86 50 L 70 60 L 50 25 L 30 60 L 14 50 Z" fill={`url(#${p}gradHolo)`} opacity="0.6" />
      <polygon points="50,12 55,20 50,28 45,20" fill="#FFF" filter={`drop-shadow(0 0 20px #FF00A0)`} />
      <rect x="20" y="72" width="60" height="4" fill="#FFF" opacity="0.3" />
    </>
  );
}

export default function TierIcon({ tier = 'unranked', size = 'md', className = '', rank }) {
  const uid = useId();
  ...
  let IconComponent = ICON_MAP[tier] || ICON_MAP.unranked;
  if (rank === 1) IconComponent = Rank1Icon;
  else if (rank >= 2 && rank <= 10) IconComponent = RankTop10Icon;
  ...
  return (
    <svg
      ...
      height={px}
      className={className}
      style={{ filter: `drop-shadow(0 4px 12px ${colors.secondary}aa)`, overflow: 'visible' }}
    >
      <IconComponent p={p} />
    </svg>
  );
