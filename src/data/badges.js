...
export const BADGE_DEFINITIONS = [
  ...
  // ─── 연속 제작 (category: 'creation') ───
  { id: 'streak_3',  emoji: '🔥', title: '작심삼일',      desc: '3일 연속 캐릭터 제작',  description: '3일 연속 캐릭터 제작',  color: 'orange',  category: 'creation' },
  { id: 'streak_7',  emoji: '🏃', title: '작심칠일',      desc: '7일 연속 캐릭터 제작',  description: '7일 연속 캐릭터 제작',  color: 'amber',   category: 'creation' },
  { id: 'streak_14', emoji: '⛓️', title: '현생을 사세요', desc: '14일 연속 캐릭터 제작', description: '14일 연속 캐릭터 제작', color: 'violet',  category: 'creation' },

  // ─── 하루 제작 (category: 'creation') ───
  { id: 'daily_2', emoji: '🌅', title: '아침부터 밤까지', desc: '하루에 캐릭터 2개 이상 제작', description: '하루 2개 이상 제작', color: 'sky',    category: 'creation' },
  { id: 'daily_4', emoji: '🪥', title: '양치?',           desc: '하루에 캐릭터 4개 이상 제작', description: '하루 4개 이상 제작', color: 'cyan',   category: 'creation' },
  { id: 'daily_6', emoji: '💉', title: '제작중독',        desc: '하루에 캐릭터 6개 이상 제작', description: '하루 6개 이상 제작', color: 'violet', category: 'creation' },
...
export function computeEarnedTitles(input) {
  ...
  for (let i = 0; i < kstDateStrings.length; i++) {
    if (i === 0) {
      streak = 1;
    } else {
      const prev = new Date(kstDateStrings[i - 1]);
      const curr = new Date(kstDateStrings[i]);
  ...
  // ─── 연속 제작 ───
  list.push({ id: 'streak_3',  emoji: D('streak_3').emoji,  title: D('streak_3').title,  desc: D('streak_3').desc ?? '',  color: 'orange', earned: maxStreak >= 3,  progress: { label: '연속 제작일', current: maxStreak, max: 3  } });
  list.push({ id: 'streak_7',  emoji: D('streak_7').emoji,  title: D('streak_7').title,  desc: D('streak_7').desc ?? '',  color: 'amber',  earned: maxStreak >= 7,  progress: { label: '연속 제작일', current: maxStreak, max: 7  } });
  list.push({ id: 'streak_14', emoji: D('streak_14').emoji, title: D('streak_14').title, desc: D('streak_14').desc ?? '', color: 'violet', earned: maxStreak >= 14, progress: { label: '연속 제작일', current: maxStreak, max: 14 } });
  ...
  const maxDailyCount = Object.values(dailyCountMap).reduce((max, v) => Math.max(max, v), 0);
  list.push({ id: 'daily_2', emoji: D('daily_2').emoji, title: D('daily_2').title, desc: D('daily_2').desc ?? '', color: 'sky',    earned: maxDailyCount >= 2, progress: { label: '하루 최대 제작', current: maxDailyCount, max: 2 } });
  list.push({ id: 'daily_4', emoji: D('daily_4').emoji, title: D('daily_4').title, desc: D('daily_4').desc ?? '', color: 'cyan',   earned: maxDailyCount >= 4, progress: { label: '하루 최대 제작', current: maxDailyCount, max: 4 } });
  list.push({ id: 'daily_6', emoji: D('daily_6').emoji, title: D('daily_6').title, desc: D('daily_6').desc ?? '', color: 'violet', earned: maxDailyCount >= 6, progress: { label: '하루 최대 제작', current: maxDailyCount, max: 6 } });
  ...
  const tsChars = charsWithTag(characters, 'ts');
  list.push({ id: 'ts', emoji: '🎭', title: '수컷암컷실격', desc: BADGE_DEFINITIONS.find(b => b.id === 'ts')?.desc ?? '', color: 'gradient', earned: tsChars.length > 0, chars: tsChars });
  const madnessChars = charsWithAnyTag(characters, ['광기', '싸이코패스', '사이코패스']);
  list.push({ id: 'madness', emoji: '👁️', title: '맑눈광', desc: BADGE_DEFINITIONS.find(b => b.id === 'madness')?.desc ?? '', color: 'violet', earned: madnessChars.length > 0, chars: madnessChars });
  const childhoodChars = charsWithTag(characters, '소꿉친구');
  list.push({ id: 'childhood', emoji: '🚲', title: '맛잘알', desc: BADGE_DEFINITIONS.find(b => b.id === 'childhood')?.desc ?? '', color: 'sky', earned: childhoodChars.length > 0, chars: childhoodChars });
  const apocalypseChars = charsWithAnyTag(characters, ['아포칼립스', '좀비']);
  list.push({ id: 'apocalypse', emoji: '🧟', title: '시켜볼게 있어', desc: BADGE_DEFINITIONS.find(b => b.id === 'apocalypse')?.desc ?? '', color: 'lime', earned: apocalypseChars.length > 0, chars: apocalypseChars });
